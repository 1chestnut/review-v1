...
ROOT = Path('/data/zkx/zkx/review1/05b_strict-map-k5m3-freeze')
...
def worker(gpu):
    ...
    for name in QUEUES[gpu]:
        ...
        with (folder / 'run.log').open('a') as log:
            process = subprocess.Popen([sys.executable, '-u', str(folder / 'run_clap_iknow.py')],
...
def launch():
    ...
    (ROOT / 'launch_status.json').write_text(json.dumps({'state':'launched','queues':QUEUES,'time':time.strftime('%F %T')}, indent=2))
