...
ROOT = Path('/data/zkx/zkx/review1/06_onehop-text-verbalization-audit')
QUEUES = {0: ['05_AudioSet', '04_DCASE17_T4'],
...
def worker(gpu):
    ...
    for name in QUEUES[gpu]:
        ...
        with (folder/'run.log').open('a') as log:
            process = subprocess.Popen([sys.executable, '-u', str(folder/'run_onehop_text_audit.py')],
                                       cwd=folder, env=env, stdout=log, stderr=subprocess.STDOUT)
            status['pid'] = process.pid; (folder/'run_status.json').write_text(json.dumps(status, indent=2))
            code = process.wait()
...
def launch():
    for gpu, queue in QUEUES.items():
        for name in queue: (ROOT/name/'run_status.json').write_text(json.dumps({'state':'queued','gpu':gpu}, indent=2))
    ...
    (ROOT/'launch_status.json').write_text(json.dumps({'state':'launched','time':time.strftime('%F %T'),'queues':QUEUES}, indent=2))
