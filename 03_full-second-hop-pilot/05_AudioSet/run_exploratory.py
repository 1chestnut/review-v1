#!/usr/bin/env python3
"""Exploratory combined second-hop pilot built on the frozen 01 one-hop setup."""
...
RESULT_FILE = RESULTS / "exploratory_results.json"
TABLE_FILE = RESULTS / "exploratory_table.csv"
        ...
        if query_head not in factory.entity_to_id:
            ...
            for middle in first.values():
                for relation in valid_relations:
                    ...
                    for tail in tails:
                        before = len(second)
                        filtered_add(second, tail, class_name, head, middle)
                        if len(second) > before:
                            paths.append({"head": head, "middle": middle,
...
def main():
    ...
    base = load_runtime()
    device = base.DEVICE
    print(f"Starting exploratory pilot: {CONFIG['dataset']}", flush=True)
    ...
    for sample_index, sample in enumerate(tqdm(samples, desc=CONFIG["dataset"], mininterval=10)):
        ...
        for class_index in top_indices:
            ...
            ids2 = evidence_index[class_index]["h2"]
            s1 = evidence_scores[ids1] if ids1 else torch.empty(0, device=device)
            s2 = evidence_scores[ids2] if ids2 else torch.empty(0, device=device)
            if not ids1:
                audit["empty_h1"] += 1
            if ids2:
            ...
            decayed_s2 = GAMMA * s2
            combined = torch.cat([s1, decayed_s2])
            score_vectors["2nd-Full"][class_index] = new_fusion(
                base_scores[class_index], combined, alpha, scale
            )
            keep_full = torch.argsort(combined, descending=True, stable=True)[:TOP_P]
            selected_h2_full = int(torch.sum(keep_full >= len(ids1))) if ids2 else 0
            audit["selected_h2_texts_full"] += selected_h2_full
            class_records.append({
                "class_index": class_index,
                "h1_count": len(ids1), "h2_count": len(ids2),
                "selected_h2_full": selected_h2_full,
            })
