...
def main():
    print("Starting " + CONFIG["dataset"], flush=True)
    base = load_base()
    device = base.DEVICE
    ...
    print("Models ready; loading dataset", flush=True)
    ...
    print(f"Dataset: {len(samples)} samples; {len(labels)} classes", flush=True)
    ...
    for sample_index in tqdm(
        ...
        for class_index in top_indices:
            ...
            kg_entity = base.get_kg_entity(kg_classes[class_index])
            query_entity = base.resolve_head_query(kg_entity, factory)
