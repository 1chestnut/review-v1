...
ROOT=Path('/data/zkx/zkx/review1');TASK=ROOT/'28_TUT2017_full6300_rerun'/'main'
...
def main(ds):
 out=TASK/'test'/ds;out.mkdir(parents=True,exist_ok=True);src=ROOT/'28_TUT2017_full6300_rerun'/'cache'/ds
 rt=mod(Path(__file__).parent/'runtime'/ds/'runtime.py');dev=rt.DEVICE;seed(42);torch.use_deterministic_algorithms(True)
 ...
 save(out/'samples.json',rows);save(out/'metrics.json',met);save(out/'protocol.json',{'role':'final evaluation','dataset_partition':'TUT2017 development (4680) + evaluation (1620)','n_samples':len(samples),'development_set':'DCASE17-T4','alpha':.3,'Nr':5,'K':5,'M':3,'kappa':100,'TopP':'All/disabled','hop':1,'seed':42,'independent_branches':True,'labels_used_by_selector':False,'selection_counts':{m:dict(c) for m,c in counts.items()}})
