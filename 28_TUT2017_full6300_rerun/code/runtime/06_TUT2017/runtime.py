...
TUT_DEV_DIR = '/data/zkx/zkx/iknow-audio/data/TUT2017/development/TUT-acoustic-scenes-2017-development'
TUT_EVAL_DIR = '/data/zkx/zkx/iknow-audio/data/TUT2017/evaluation/TUT-acoustic-scenes-2017-evaluation'
...
def load_dataset():
    records = []
    classes = set()
    for split_name, split_dir in [('development', TUT_DEV_DIR), ('evaluation', TUT_EVAL_DIR)]:
        with open(os.path.join(split_dir, 'meta.txt'), 'r', encoding='utf-8') as f:
            for line in f:
                parts = line.strip().split()
                if len(parts) < 2:
                    continue
                audio_rel_path, scene_label = (parts[0], parts[1])
                records.append((split_name, split_dir, audio_rel_path, scene_label))
...
def iter_samples(dataset):
    ...
    for split_name, split_dir, audio_rel_path, scene_label in dataset['records']:
        true_indices = [class_to_idx[scene_label]] if scene_label in class_to_idx else []
        yield {
            'audio_path': os.path.join(split_dir, audio_rel_path),
