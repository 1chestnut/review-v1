...
All methods for a given dataset run sequentially on the same isolated GPU, with the same 200 evenly spaced samples, identical sample order and immutable caches. Different datasets may run concurrently on three separate GPUs; the physical GPU identity is recorded and no cross-GPU latency comparison is used to claim a method advantage. Before every live CLAP encoding call, the RNG is reset from the immutable identity of that audio batch, preventing rotated method order from changing random crop/padding behavior. Both batch sizes 1 and 32 are measured after a 20-sample warm-up. Every setting is repeated five times; method order is rotated and CUDA is synchronized. Top-1 agreement across live re-encoding repeats must be at least 99.5%. Complete ranking hashes are retained for audit but are not required to be identical because numerically near-tied, low-ranked classes can exchange order without changing the evaluated prediction.
