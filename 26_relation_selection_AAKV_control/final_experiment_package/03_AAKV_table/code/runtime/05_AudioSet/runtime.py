...
CLASS_LABELS_CSV = '/data/zkx/zkx/iknow-audio/data/audioset/class_labels_indices.csv'
...
def load_dataset():
    class_df = pd.read_csv(CLASS_LABELS_CSV).sort_values('index')
    mid_to_name = dict(zip(class_df['mid'], class_df['display_name']))
    kg_classes = class_df['display_name'].tolist()
    class_to_idx = {cat: idx for idx, cat in enumerate(kg_classes)}
    df = pd.read_csv(AUDIO_LABELS_CSV)
    return {'df': df, 'mid_to_name': mid_to_name, 'label_classes': kg_classes, 'kg_classes': kg_classes, 'class_labels_set': {name.lower() for name in kg_classes}, 'class_to_idx': class_to_idx, 'total': len(df)}
