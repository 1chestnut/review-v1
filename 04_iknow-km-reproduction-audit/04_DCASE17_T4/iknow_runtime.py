...
DCASE_CSV = '/data/zkx/zkx/iknow-audio/data/DCASE17-T4/my_evaluation_dataset.csv'
DCASE_AUDIO_DIR = '/data/zkx/zkx/iknow-audio/data/DCASE17-T4/audio'
...
def get_kg_entity(class_name):
    mapping = {'ambulance (siren)': 'ambulance', 'fire engine, fire truck (siren)': 'fire engine', 'police car (siren)': 'police car', 'air horn, truck horn': 'horn', 'civil defense siren': 'siren', 'reversing beeps': 'beep', 'car passing by': 'car'}
...
def load_dataset():
    label_classes = ['ambulance (siren)', 'bicycle', 'bus', 'car', 'car alarm', 'car passing by', 'civil defense siren', 'fire engine, fire truck (siren)', 'motorcycle', 'police car (siren)', 'reversing beeps', 'screaming', 'skateboard', 'train', 'train horn', 'truck', 'air horn, truck horn']
    class_to_idx = {cat.lower(): idx for idx, cat in enumerate(label_classes)}
    df = pd.read_csv(DCASE_CSV)
    return {'df': df, 'label_classes': label_classes, 'kg_classes': label_classes, 'class_labels_set': {name.lower() for name in label_classes}, 'class_to_idx': class_to_idx, 'total': len(df)}
