#!/usr/bin/env python3
"""Task18D: label-free reciprocal-rank consensus relation selector."""
import os
...
METHODS=["CLAP","Frozen-iKnow","Abs-Top3","Margin-Top1","Margin-Top3",
         ...
         "Consensus-Margin-Top3","Ranked-Consensus-Top1",
         "Ranked-Consensus-Top3","Relation-Oracle-analysis-only"]
...
def main():
 ...
 a18=json.loads(Path(x.task18a_result).read_text());oracle={int(v["sample_index"]):int(v["oracle_rank"]) for v in a18["rows"]};oracle_rels={int(v["sample_index"]):list(v["oracle_relations"]) for v in a18["rows"]};cache=torch.load(x.task18a_cache,map_location="cpu")
 ...
 for si in tqdm(range(int(st["next"]),len(samples)),total=len(samples),initial=int(st["next"]),desc=cfg["dataset"],mininterval=10):
  ...
  # Parameter-free ranked voting: every relation contributes reciprocal-rank
  # support to every candidate class.  Relations are then ordered by how highly
  # they rank the aggregate consensus class, with margin only breaking ties.
  rr_vote=torch.zeros_like(bs);rel_class_rank={}
  for r,z in single.items():
   order=torch.argsort(z,descending=True)
   inv=torch.empty_like(order,dtype=torch.long);inv[order]=torch.arange(1,order.numel()+1,device=order.device)
   rel_class_rank[r]=inv;rr_vote += 1.0/inv.float()
  ranked_class=int(torch.argmax(rr_vote).item())
  ranked_order=sorted(rels,key=lambda r:(int(rel_class_rank[r][ranked_class].item()),-margin[r],r))
  ...
  sel["Ranked-Consensus-Top1"]=ranked_order[:1];sel["Ranked-Consensus-Top3"]=ranked_order[:3]
  ...
  rows.append({"sample_index":si,"audio_path":s["audio_path"],"true_indices":[int(i) for i in s["true_indices"]],"audio_seed":sd,"selected_relations":sel,"consensus_class_index":cons_class,"consensus_votes":maxvote,"ranked_consensus_class_index":ranked_class,"ranked_consensus_class_score":float(rr_vote[ranked_class].item()),"oracle_relations_analysis_only":oracle_rels[si],"ranks":ranks})
  if (si+1)%20==0:save(pp,{"next":si+1,"rows":rows})
 met={m:metric([v["ranks"][m] for v in rows]) for m in METHODS};res={"completed":True,"dataset":cfg["dataset"],"n":len(rows),"protocol":{"hop":1,"base":"Task05b","seed":"Task12 SHA256 path rule","top_k":K,"top_m":cfg["top_m"],"text":"class_name, tail","labels_used_by_selector":False,"softmax_scale":scale,"only_changed_variable":"hard majority consensus replaced by parameter-free reciprocal-rank consensus","ranked_vote":"sum_r 1/rank_r(class)","relation_order":"rank of ranked-consensus class, then classification margin"},"metrics":met,"selection_counts":{m:dict(c) for m,c in counts.items()},"rows":rows,"created_at":time.strftime("%F %T")};save(out/"ranked_consensus_results.json",res)
 lines=["method,Hit@1,Hit@3,Hit@5,MRR"]+[f"{m},{met[m]['Hit@1']:.8f},{met[m]['Hit@3']:.8f},{met[m]['Hit@5']:.8f},{met[m]['MRR']:.8f}" for m in METHODS];(out/"ranked_consensus_table.csv").write_text("\n".join(lines)+"\n");save(pp,{"next":len(samples),"rows":rows});print(json.dumps(met,indent=2))
