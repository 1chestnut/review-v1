#!/usr/bin/env python3
"""Task 10: unified audio-aware Qwen verbalization against frozen Task 06."""
...
TASK_ROOT = ROOT / "10_qwen-audioaware-verbalization"
MODEL_PATH = Path("/data/zkx/zkx/iknow-audio/data/model/千问")

SYSTEM_PROMPT = """You are a faithful knowledge-graph verbalizer for an audio-text matching model.
Convert exactly one knowledge-graph triple into one short, natural English sentence with an explicit acoustic context.

Rules:
1. Explicitly place the supplied head in an acoustic context. Choose whichever of "the sound of {head}", "the audio event {head}", or "the acoustic category {head}" preserves the supplied relation most faithfully.
2. Use only information explicitly contained in the triple.
3. Preserve the head entity, relation meaning, relation direction, and tail entity.
4. Preserve the original wording of the head and tail. Do not pluralize, abbreviate, replace, or omit either entity.
5. Do not add acoustic properties, causes, locations, purposes, examples, explanations, or external knowledge.
6. Do not correct or reinterpret the factual content, even if it seems unusual.
7. Add only grammatical function words such as articles, prepositions, and forms of "be".
8. Silently verify that the sentence expresses exactly the supplied triple.
9. Output only one final sentence, without reasoning, labels, headers, or quotation marks."""

REPAIR_PROMPT = """Your preceding sentence failed an automatic fidelity check.
Rewrite it as exactly one short English sentence that explicitly places {head} in an acoustic context, using "the sound of", "the audio event", or "the acoustic category" as appropriate for the supplied relation.
Copy the supplied head and tail text exactly and explicitly express the supplied relation in its original direction.
Do not add, correct, explain, or infer anything. Output only the sentence."""
...
def validate(head, relation, tail, sentence):
    ...
    s_norm = norm(clean)
    acoustic_markers = ("sound", "audio event", "acoustic category")
    if not any(marker in s_norm for marker in acoustic_markers): reasons.append("acoustic_context_missing")
...
def generate_dataset(dataset_dir):
    ...
    for i, row in enumerate(tqdm(triples, desc=f"Qwen {dataset_dir}", mininterval=10)):
        ...
        system = SYSTEM_PROMPT.replace("{head}", str(row["head"]))
        repair_prompt = REPAIR_PROMPT.replace("{head}", str(row["head"]))
        ...
        if raw_reasons:
            ...
            if repair_reasons:
                verified = f"The audio event {row['head']} has the relation {row['relation']} with {row['tail']}."
                outcome = "universal_fallback"
            else:
...
def evaluate_dataset(dataset_dir, generated):
    ...
    methods = ["CLAP", "Manual-Template", "Qwen-AudioAware"]
    forms = [("manual", "Manual-Template"), ("qwen_verified", "Qwen-AudioAware")]
    ...
    (results_dir/"qwen_audioaware_results.json").write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding="utf-8")
    lines=["method,Hit@1,Hit@3,Hit@5,MRR"]
    for m in methods:
        x=table[m]; lines.append(f"{m},{x['Hit@1']:.8f},{x['Hit@3']:.8f},{x['Hit@5']:.8f},{x['MRR']:.8f}")
    (results_dir/"qwen_audioaware_table.csv").write_text("\n".join(lines)+"\n",encoding="utf-8")
