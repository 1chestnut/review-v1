...
To view source and compiled paper side by side: click inside `main.tex`, compile with LaTeX Workshop (or save it; auto-build is on), then run **LaTeX Workshop: View LaTeX PDF** from the Command Palette (`Ctrl+Shift+P`) or use the two-step chord `Ctrl+L`, release, then `Alt+V`. Do not use `Ctrl+Alt+V` here: on this computer it is intercepted by a speech-to-text shortcut. The project enables LaTeX Workshop's alternate keymap, and its viewer opens the PDF in a right-hand editor group. The PDF comes from `build/main.pdf`; `drafts_zh/*.md` is an unapproved Chinese working draft and is not yet part of the compiled manuscript.
