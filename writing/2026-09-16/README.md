...
The PDF is generated beside `main.tex` as `main.pdf`, so it appears in VS Code's left-hand Explorer. Automatic compilation is off. After editing, right-click inside the `main.tex` editor and choose **Build LaTeX project** from the LaTeX Workshop context menu; then click `main.pdf` in the Explorer to view the updated paper. The build button in the editor title bar is another mouse-only option. You can drag the PDF tab to the right to keep source and PDF side by side. Do not use `Ctrl+Alt+V` here: on this computer it is intercepted by a speech-to-text shortcut. `drafts_zh/*.md` is an unapproved Chinese working draft and is not yet part of the compiled manuscript.
