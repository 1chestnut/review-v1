...
To view source and compiled paper side by side: open `main.tex`, compile with LaTeX Workshop (or save it; auto-build is on), then run **LaTeX Workshop: View LaTeX PDF** from the Command Palette or press `Ctrl+Alt+V`. This project's viewer setting opens the PDF in a right-hand editor group. The PDF comes from `build/main.pdf`; `drafts_zh/*.md` is an unapproved Chinese working draft and is not yet part of the compiled manuscript.
